...
Recently Kingma and Welling \cite{kingma2013auto} submitted a paper to ICLR 2014, in this report we describe the novelty that is introduced and look at several applications. Specifically, we try to replicate the results of Kingma and Welling by implementing the auto-encoder that they describe in the paper. After this replication step, that confirms our implementation works as it should, we test if using the auto-encoder to transform our data improves classification. Furthermore, we test the hypothesis that performing classification on the features is more robust to overfitting. 
...
\centering
\begin{minipage}{0.5\textwidth}
\includegraphics[height=3.2in,width=3.2in]{manifoldFF.png}\caption{Visualization of generated data from an auto-encoder trained two-dimensional latent space on the Frey Face dataset}
\end{minipage}
\end{figure}
