...
The problem class associated with AEVB is that where data $\mathbf{X}$ is assumed to be produced by some underlying process involving (continuous) hidden variables $\mathbf{Z}$. We can express the marginal probability over $\mathbf{X}$ by reformulating Bayes' Rule:
...
Introducing $q(Z|X)$, parameterized by $\phi$, as an approximation of the posterior $P(Z|X)$, Kingma and Welling \cite{kingma2013auto} derive an expression for the Monte Carlo estimate of the lower bound $\mathcal{L}$ of the marginal likelihood $P(X)$:

\begin{align}
\mathcal{L}(\theta ,\phi ,  \mathbf{x^{(i)}}) \backsimeq \frac{1}{L} \sum_{l=1}^{L} \text{log } p_{\theta} (x^{(i)}|z^{(l)})+ \text{log }p(z^{(l)}	)-\text{log }q_{\phi}(z^{(l)}|x^{(i)})
\end{align}

%Check second sentence, starting with "Therefore..."
Here, $z$ is sampled $L$ times from $q_{\phi}(z|x)$ for each data point $x^{(i)}$. Therefore, $z$ depends on parameters $\theta$, differentiating $\mathcal{L}$ w.r.t. $\phi$ will lead to a gradient which is influenced by the current parameters $\phi$. 
As a solution to this problem, \cite{kingma2013auto} reparameterizes the samples of $z$ from $q(z|x)$ as
\begin{align}
z = g_\phi(\mathbf{\epsilon},\mathbf{x}) \text{  with  } \mathbf{\epsilon} \sim p(\mathbf{\epsilon}) 
\end{align} 

Now, once we have a sample $\epsilon$, the MC-estimate of $\mathcal{L}$ (equation (1) ) does not depend on a \textit{sampled} $z$, but on a  $z$ which is \textit{calculated deterministically}. Thus, $\mathcal(L)$ can now be differentiated w.r.t all the parameters $\theta$ and $\phi$, enabling parameter optimization by means of stochastic gradient ascent.

\subsection{Modelling the conditional distributions}
%Easy fix, but check Durk's comment
For our experiments, we use a neural networks with a single hidden layer, similar to \cite{kingma2013auto}. $q(Z|X)$ is modeled as a multivariate Gaussian with diagonal covariance. Obtaining the parameters for the multivariate Gaussian $q(\mathbf{z}|\mathbf{x})$ from data point $\mathbf{x}$ is now done through
\begin{align}
\mathbf{\mu_z} = f_2(\mathbf{W}_2^T \cdot f_1(\mathbf{W}_1^T\mathbf{x}+\mathbf{b_1})+\mathbf{b_2}) \\
\mathbf{\sigma_z} = f_3(\mathbf{W}_3^T \cdot f_1(\mathbf{W}_1^T\mathbf{x}+\mathbf{b_1})+\mathbf{b_3})
\end{align} 
Where $f_1$ through $f_3$ denote the activation functions.\\
$P(X|Z)$ can be modeled in this way too if the data $\mathbf{X}$ is continuously valued. If the values are binary, it can be modeled by a multivariate Bernoulli distribution.  These neural networks are powerful, in the sense that they are universal approximators, i.e. they can approximate any function of their input. They can be efficiently trained with backpropagation of error derivatives, especially if batch optimization is used. This is done by expanding $x$, $\mathbf{\mu_z}$ and $\mathbf{\sigma_z}$ to matrices to acquire gradients of multiple data points at once for one parameter update step.\\
...
%Auto encoder discussion above here!
...
%Update these scores by using the new trained chinese character encoder (on small set)?
...
One of the advantages of extracting a set of relevant features of data, in general, is that performing classification on these features is more robust to overfitting than performing classifications on the original data. This robustness can be explained by the fact that the weights activate on a clump of pixels, which is a more discriminative property of an image then the raw pixel values would be.  Such an activation can be seen in figure 7, although this visualization is of the weights of the decoder the effect is the same for the weights of the encoder.
